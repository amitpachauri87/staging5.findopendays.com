...
class ListingLocationStore
{
    ...
    public function store($update = false)
    {
        ...
        if (! $this->latitude || ! $this->longitude) {
            // Save none address meta.
            update_post_meta($this->post->ID, '_qibla_mb_map_location_all_data', '0,0:none');
            update_post_meta($this->post->ID, '_qibla_mb_map_location_lat', 0);
            update_post_meta($this->post->ID, '_qibla_mb_map_location_lng', 0);
            // Set the address term.
            wp_set_object_terms($this->post->ID, array(), ListingLocation::LISTING_ADDRESS_TAXONOMY, false);
            ...
            $allData = $this->latitude . ',' . $this->longitude . ':' . $this->address;

            if ($update) {
                update_post_meta($this->post->ID, '_qibla_mb_map_location_all_data', $allData);
            } else {
                add_post_meta($this->post->ID, '_qibla_mb_map_location_all_data', $allData, true);
            }
