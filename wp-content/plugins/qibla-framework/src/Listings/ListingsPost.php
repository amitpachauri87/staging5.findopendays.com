...
class ListingsPost implements ListingsPostInterface, LocalizableInterface, ThumbnailLiableInstance, IconizeInterface, ListingTermInterface, SubTitleInterface, ListingsExtraInterface
{
    ...
    {
        /**
         * Filter default icon based on several custom post types.
         *
         * @since 2.3.0
         */
        $iconCompact = apply_filters('qibla_listings_post_default_icon', $default, $this->post);

        // Retrieve the first term of listing_categories taxonomy.
        // Icons for listings are grub down by the first term.
        $terms = get_the_terms(
            $this->ID(),
            apply_filters('qibla_listings_post_taxonomy_for_icon', 'listing_categories', $this->post)
        );
    ...
    public function color()
    {
        ...
        $terms = get_the_terms(
            $this->ID(),
            apply_filters('qibla_listings_post_taxonomy_for_color', 'listing_categories', $this->post)
        );

        // Default color.
        $termColor = F\getThemeOption('colors', 'main');

        if (! is_wp_error($terms) && $terms) {
            $term = wp_list_pluck($terms, 'term_id');
            // Get the Term Color from term meta.
            $termColor = F\getTermMeta('_qibla_tb_terms_color', $term[0], $termColor);
        }
    ...
    public function thumbnail()
    {
         ...
         * @since 2.4.0
         */
        $size = apply_filters('qibla_listings_post_thumbnail_size', 'qibla-post-thumbnail-loop');
