...
class Store extends AbstractMetaboxStore
{
    ...
    public function meta($term_id, $tt_id, $taxonomy)
    {
        ...
        if (! current_user_can('edit_posts') && ! User::isListingsManager()) {
            wp_die(esc_html__('You are not allowed to store term meta.', 'qibla-framework'));
        }
        ...
        foreach ($this->metaBoxes as $metaBox) :
            ...
            // @todo check and remove edittag action if don't used.
            if ('add-tag' === $action || 'edittag' === $action || 'editedtag' === $action && wp_get_referer()) {
