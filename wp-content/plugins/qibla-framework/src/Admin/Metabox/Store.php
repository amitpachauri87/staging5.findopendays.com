...
use QiblaFramework\Admin\AbstractMetaboxStore;
...
class Store extends AbstractMetaboxStore
{
    ...
    public function meta($postID, $post, $update)
    {
        ...
        if (! current_user_can('edit_posts') && ! User::isListingsManager()) {
            wp_die(esc_html__('You are not allowed to store post meta.', 'qibla-framework'));
        }
        ...
        // Do nothing if the post status is 'auto-draft,trash,inherit' or WordPress is performing an untrash action.
            ...
            if (! empty($fields)) :
                ...
                if (is_array($validate) && ! empty($validate)) {
                    foreach ($validate['valid'] as $name => $value) :
                         ...
                         */
                        do_action('qibla_fw_metabox_after_store_meta', $name, $value, $post, $update, $oldValue, $this);
                    endforeach;
                }
