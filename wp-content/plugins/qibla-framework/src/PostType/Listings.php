...
class Listings extends AbstractPostType
{
    ...
    public function __construct()
    {
        parent::__construct(
            'listings',
            esc_html__('Listing', 'qibla-framework'),
            esc_html__('Listings', 'qibla-framework'),
            array(
                // Filter the location since other plugins may hide this or this may hide other menu items.
                'menu_position'   => apply_filters('qibla_post_type_menu_position_listings', 5),
                'menu_icon'       => 'dashicons-store',
                'has_archive'     => true,
                'description'     => F\getThemeOption('listings', 'listings_archive_description', true),
    ...
    public function columns($columns)
    {
        ...
        $columns = array_merge($columns, array(
            'location' => esc_html_x('Location', 'admin-table-list', 'qibla-framework'),
            'category' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Category', 'admin-table-list', 'qibla-framework')
            ),
//            'amenities' => esc_html_x('Amenities', 'admin-table-list', 'qibla-framework'),
            'featured' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Featured', 'admin-table-list', 'qibla-framework')
            ),
//            'gallery'  => esc_html__('Gallery', 'qibla-framework'),
            'creator'  => sprintf(
                '<span class="has-icon" data-icon="">%s</span>',
                esc_html_x('Author', 'admin-table-list', 'qibla-framework')
            ),
            'comments' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Reviews', 'admin-table-list', 'qibla-framework')
            ),
        ));
...
//                    '<span>' . esc_html__('Yes', 'qibla-framework') . '</span><i class="la la-check la--active">' :
//                    '<span>' . esc_html__('No', 'qibla-framework') . '</span><i class="la la-check">';
//                break;
            case 'category':
...
//                break;
            case 'featured':
                ...
                echo 'on' === $meta ?
                    '<span>' . esc_html__('Yes', 'qibla-framework') .
                    '</span><i class="la la-star dlicon dlicon--active">' :
                    '<span>' . esc_html__('No', 'qibla-framework') .
