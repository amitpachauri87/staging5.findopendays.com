...
class Context
{
     ...
     * @since 2.0.0
     *
     * @var \QiblaFramework\ListingsContext\Types The listings custom post types
     */
    private $types;
     ...
     * @param \QiblaFramework\ListingsContext\Types $types The listings custom post types.
     */
    ...
    /**
     * Listings Archive Type
     *
     * @since 2.5.1
     *
     * @return null|string
     */
    public function listingsArchiveType()
    {
        $type = null;
        ...
        if ($obj instanceof \WP_Post_Type) {
            ...
            $type = $this->listingTypeFromTax($obj->taxonomy);
        }
    ...
    public function context()
    {
        ...
        if (isset($_REQUEST['post_type'])) {
            $postType = F\filterInput($_REQUEST, 'post_type', FILTER_SANITIZE_STRING);
            // Is multi post type?
            if(is_array($_REQUEST['post_type'])) {
                $postType = in_array(get_post_type(), $_REQUEST['post_type'], true) ? get_post_type() : 'listings';
            }
