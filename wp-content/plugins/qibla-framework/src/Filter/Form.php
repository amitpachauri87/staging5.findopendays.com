...
use QiblaFramework\Form\Abstracts;
...
class Form extends Abstracts\Form
{
     ...
     * @since 2.0.0
     *
     * @var \QiblaFramework\Collection A collection of filters
     */
    private $filters;
     ...
     *
     * @var \QiblaFramework\Listings\Context The context in which the form works
     */
    private $context;
     ...
     * @since 2.0.0
     *
     * @var \QiblaFramework\Filter\GeocodeFilter The geocode filter instance
     */
    ...
    public function getHtml()
    {
        ...
        /**/
        $output = sprintf(
            '<form action="%s" method="%s" name="%s"%s><div class="dl-field dl-field--select"><input type="text" name="custom_text_search" placeholder="Search By Name" id="custom_text_search"></div>',
         ...
         * @param \QiblaFramework\Filter\Form $this    The instance of this form.
        ...
        if (! empty($filters['hidden'])) {
            $button = sprintf(
                ...
                esc_html__('Other filters', 'qibla-framework')
            );
            ...
            if ('no' === apply_filters('qibla_disable_form_filter_hidden_fields', 'no')) {
                ...
                $output .= sprintf(
                    ...
                    "dlbtn dlbtn--tiny dlbtn--ghost btn-clear {$scopeClass}__action {$scopeClass}__action--clear",
                    esc_html__('Close', 'qibla-framework')
                );
                // - Submit Button
                $output .= sprintf(
                    '<input type="submit" class="%s" value="%s" data-action="update" />',
                    "dlbtn dlbtn--tiny {$scopeClass}__action {$scopeClass}__action--update",
                    esc_html__('Update', 'qibla-framework')
                );
