...
class MultiCheck extends Checkbox
{
    ...
    public function __construct($args)
    {
        ...
        if (! isset($args['options']['all']) && empty($args['exclude_all'])) {
            $args['options'] = array_merge(array('all' => esc_html__('All', 'qibla-framework')), $args['options']);
        }
    ...
    public function getHtml()
    {
        ...
        foreach ($options as $value => $label) {
            if (is_array($label) && array_key_exists('title', $label)) {
                foreach ($label as $key => $subList) {
                    if ('title' === $key) {
                        $output .= '<li class="multicheck-list__title">';
                        $output .= '<span>' . $subList . '</span>';
                        $output .= '<ul class="multicheck-list__sub">';
                    } else {
                        $output .= '<li class="multicheck-list__item">';
                        $output .= $this->makeInput($key, $subList);
                        $output .= '</li>';
                    }
                }
                $output .= '</ul></li>';
            } else {
                $output .= '<li class="multicheck-list__item">' . $this->makeInput($value, (string)$label) . '</li>';
            }
        }
         ...
         */
        $output = apply_filters('qibla_fw_type_multicheck_output', $output, $this);
