...
if (RevolutionSlider::sliderIsActive()) {
    ...
    $fields = array(
         ...
         * @since 1.6.0
         */
        'qibla_tb_jumbotron_slider:select' => $fieldFactory->termbox(array(
            'type'        => 'select',
            'select2'     => true,
            'name'        => 'qibla_tb_jumbotron_slider',
            'value'       => F\getTermMeta('_qibla_tb_jumbotron_slider', self::$currTerm->term_id, 'none'),
            'options'     => $slider->getSliderList(),
            'label'       => esc_html__('Choose a slider', 'qibla-framework'),
            'description' => esc_html__('The slider will overrider the hero image option.', 'qibla-framework'),
 ...
 */
return apply_filters('qibla_tb_inc_jumbotron_slider_fields', $fields);
