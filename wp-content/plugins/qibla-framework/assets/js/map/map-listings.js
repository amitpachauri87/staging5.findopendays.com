    ...
    function (
        ...
        DL.Listings.Filter = {
            ...
            init: function () {
                ...
                if (!_.isEmpty(jsonListings) && !_.isEmpty(jsonListings.posts)) {
                    ...
                    if (this.visibleMap()) {
                        ...
                        var posts = jsonListings.posts;
                        var i = 0, itemsCollection = [];

                        // Remove no post whit no address.
                        _.forEach(jsonListings.posts, function (item) {
                            if (0 !== item.location.latLng[0] && 0 !== item.location.latLng[1]) {
                                itemsCollection[i] = item;
                                i++;
                            }
                        });

                        // Set itemsCollection in posts.
                        if (itemsCollection.length >= 0) {
                            posts = itemsCollection;
                        }

                        // Set the POI.
                        DlMap.markerCollection.add(posts);
            ...
            filter: function (e) {
                ...
                var elementToScrollTo = document.body.classList.contains('is-safari') ||
                                        ...
                                        document.body.classList.contains('is-mobile') ? 'html,body' : 'html';
                        ...
                        success: function (collection, data, options) {
                            ...
                            if (!data.foundPosts.number) {
                                ...
                                if (DL.Listings.Filter.visibleMap()) {
                                    ...
                                    // Set Collection for Map.
                                    DlMap.markerCollection.reset();
                                    _.forEach(collection.models, function (model) {
                                        if (0 !== model.attributes.location.latLng[0] && 0 !== model.attributes.location.latLng[0]) {
                                            // Insert the current model within the marker collection.
                                            DlMap.markerCollection.add(model.attributes);
                                        }
                                    });
                            ...
                            var clearBTN = document.querySelector('.dlform-filter__action--clear');
                            if (clearBTN) {
                                clearBTN.addEventListener('click', DL.Listings.FilterToggler.toggle);
                            }
                        }.bind(this)
                    });
                }.bind(this));
            ...
            construct: function (postUrl, userPosition, formFilter) {
                ...
                // no filters no map and not use short-code.
                ...
                if (formFilter) {
                    ...
                    if (767 >= window.innerWidth) {
                        padding = formFilter.offsetHeight + offset;
                    } else if (1024 > window.innerWidth) {
                        padding = formFilter.offsetHeight;
                    } else {
                        padding = 0;
                    }
                    formFilter.nextElementSibling.style.paddingTop = padding + 'px';
                }
        ...
        DL.Listings.FilterToggler = {
            ...
            clearFilters: function (evt) {
                ...
                // Clear Checkboxes.
                var checkboxes = this.form.querySelectorAll('[type=checkbox]');
...
//            },
        ...
        window.addEventListener('load', function () {
            ...
            filter && filter.init();
            // Call the filter if there is no document.referrer (usually at the back history).
             history.pushState({url: window.location.href}, window.location.href, '');
             if (history.state.url && ! document.referrer) {
                    filter.filter('select2change');
             }
        ...
        /**
         * Toolbar height style for toolbar
         *
         * @since 2.5.1
         *
         * @param formFilter
         */
        function toolbarStyle(formFilter)
        {
            if (!formFilter) {
                return;
            }
        ...
        window.addEventListener('resize', function (e) {
            ...
            var mapResize = setTimeout(function () {
                // Form filter.
                var formFilter = document.getElementById('dlform_filter');
                if (!formFilter) {
                    return;
                }
                // Toolbar height.
                toolbarStyle(formFilter);
            }, 100);
        });
    }(
        jQuery,
