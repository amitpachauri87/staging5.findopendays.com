...
;(
    function (_, DL, dllocalized)
    {
        ...
        function searchIcon()
        {
            var input = document.getElementById('qibla_opt-icon-search');
            ...
            input.addEventListener('keyup', function () {
                var icon,
                    list = document.getElementById('dltype-current-icons-grid'),
                    items = list.getElementsByTagName('li'),
                    filter = this.value.toUpperCase();

                _.forEach(items, function (item) {
                    icon = item.getAttribute('data-icon-value');
                    icon = icon.replace(icon + '::', '');

                    if (icon.toUpperCase().indexOf(filter) > -1) {
                        item.style.display = '';
                    } else {
        ...
        {
            var input = document.getElementById('qibla_opt-icon-search'),
                grid = document.querySelector('.dltype-current-icons-grid');
            ...
            search.setAttribute('id', 'qibla_opt-icon-search');
            search.setAttribute('placeholder', dllocalized.searchIconPlaceholder);

            grid.insertAdjacentHTML('beforebegin', search.outerHTML);
        }
        ...
        _.forEach(iconTypes, function (list)
        {
            ...
            if (vendorsList) {
                ...
                {
                    var search = document.getElementById('qibla_opt-icon-search');
                ...
                currIcon.addEventListener('click', function (e)
                {
                    ...
                    if ('none' !== vendorsList.value) {
                        ...
                        if (
                            currGrid && 'block' === currGrid.style.display &&
    ...
    }(_, window.DL, window.dllocalized)
);