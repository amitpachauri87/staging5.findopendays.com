...
{
    var i = 0, markers = [];
    _.forEach(opt_markers, function (marker) {
        if (0 !== marker.latLng.lat() && 0 !== marker.latLng.lng()) {
            markers[i] = marker;
            i++;
        }
    });
...
CustomMarkerClusterer.prototype.addToClosestCluster_ = function (marker)
{
    ...
    if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)) {
        // Add offset.
        if (0 === distance) {
            marker.setPosition(new google.maps.LatLng(pos.lat() + 0.0003, pos.lng() + 0.0006))
        }
