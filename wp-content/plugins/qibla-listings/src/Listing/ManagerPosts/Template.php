...
namespace QiblaListings\Listing\ManagerPosts;
...
use QiblaListings\Listing\UserListingsPosts;
...
class Template
{
    ...
    public function getData()
    {
        ...
        foreach ($this->posts as $post) :
            ...
            $newPost->thumbnail  = Fw\getPostThumbnailAndFallbackToJumbotronImage($post, array(64, 64));
            $newPost->permalink  = $post->guid;
            $newPost->postTitle  = $post->post_title;
            $newPost->postType   = $post->post_type;
            ...
            $newPost->expirationDate = (
                ...
                esc_html__('Never', 'qibla-listings') :
            ...
            $newPost->status = (ListingsPost::EXPIRED_STATUS === $post->post_status) ?
                esc_html_x('Qibla Expired', 'manager-posts', 'qibla-listings') :
                $post->post_status;
            ...
            if ('none' !== $package) {
                ...
                if ('listings' !== $post->post_type) {
                     ...
                     * @since 2.3.0
                     */
                    $postTypePackage = apply_filters('qibla_listings_post_type_package', $postTypePackage, $post->post_type);
                }

                // Get package.
                $package = F\getPostByName($package, $postTypePackage);
