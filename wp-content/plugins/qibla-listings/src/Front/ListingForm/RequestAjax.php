...
class RequestAjax extends AbstractRequestFormAjax
{
    ...
    public static function handleFilter()
    {
        ...
        $package     = F\filterInput($_POST, PackageFormBuilder::FORM_PACKAGE_KEY, FILTER_SANITIZE_STRING);
        $packageType = F\filterInput($_POST, PackageFormBuilder::FORM_PACKAGE_POST_TYPE, FILTER_SANITIZE_STRING);

        // Retrieve the package by the POST request, needed to build the form to send to the constructor.
        if ($package && 'listing_package' === $packageType) {
            $package = Fw\getPostByName($package, 'listing_package');
        }

        // Retrieve the action to perform.
        // @codingStandardsIgnoreStart
        $action = F\filterInput(
            $_POST,
        ...
        if ($package instanceof \WP_Post && 'listing_package' === $packageType) {
