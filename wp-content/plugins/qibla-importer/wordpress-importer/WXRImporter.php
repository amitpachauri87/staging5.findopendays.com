<?php
namespace WXRImporter;

require_once untrailingslashit(ABSPATH) . '/wp-admin/includes/class-wp-importer.php';
...
class WXRImporter extends \WP_Importer
{
     ...
     * Sanitize the GUID
     *
     * Sanitize the value for the GUID by convert html entities because there is no a unique
     * way to store this data into the database. Every plugin store this data has the want.
     *
     * @since  1.2.1
     * @access protected
     *
     ...
     *
     * @return string The sanitized guid value
     */
    protected function makeUniqueGUID($guid)
    {
        $sanitizedGuid = htmlentities2($guid);
        $sanitizedGuid = wp_hash($sanitizedGuid);
    ...
    protected function get_reader($file)
    {
        ...
        if (! $status) {
            return new \WP_Error('wxr_importer.cannot_parse',
                __('Could not open the file for parsing', 'wordpress-importer'));
        }
                ...
                case 'wp:wxr_version':
                    ...
                    if (version_compare($this->version, self::MAX_WXR_VERSION, '>')) {
                        $this->logger->warning(sprintf(
                            __('This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.',
                                'wordpress-importer'),
                            $this->version,
    ...
    protected function import_start($file)
    {
        if (! is_file($file)) {
            return new \WP_Error('wxr_importer.file_missing',
                __('The file does not exist, please try again.', 'wordpress-importer'));
        }
...
//					$this->mapping['comment'][ $original_id ] = $exists;
                    $this->mapping['comment'][$original_id] = true;
                    continue;
                }
            }
    ...
    protected function parse_term_node($node, $type = 'term')
    {
        ...
        switch ($type) {
            case 'category':
                $term['id']          = 'wp:term_id';
                $term['slug']        = 'wp:category_nicename';
                $term['parent']      = 'wp:category_parent';
                $term['name']        = 'wp:cat_name';
                $term['description'] = 'wp:category_description';
                $term['taxonomy']    = null;
            ...
            case 'tag':
                $term['id']          = 'wp:term_id';
                $term['slug']        = 'wp:tag_slug';
                $term['parent']      = null;
                $term['name']        = 'wp:tag_name';
                $term['description'] = 'wp:tag_description';
                $term['taxonomy']    = null;
            ...
            case 'term':
                $term['id']               = 'wp:term_id';
                $term['term_slug']        = 'wp:term_slug';
                $term['term_parent']      = 'wp:term_parent';
                $term['term_name']        = 'wp:term_name';
                $term['term_description'] = 'wp:term_description';
                $term['term_taxonomy']    = 'wp:term_taxonomy';
                break;
        }
        ...
        foreach ($node->childNodes as $child) {
            ...
            $key = array_search($child->tagName, $term);
            if ($key) {
                switch ($key) {
                    case 'meta':
                        $currMeta               = $this->parse_meta_node($child);
                        $meta[$currMeta['key']] = $currMeta['value'];
                        break;
    ...
    protected function process_term($data, $meta)
    {
        ...
        foreach ($data as $key => $value) {
            if (isset($allowed[$key]) && isset($meta[$key])) {
                $termdata[$key] = $meta[$key];
            }
        }
        ...
        if (! is_wp_error($result)) {
            foreach ($meta as $key => $value) {
                ...
                // Do not create meta if all ready exists.
                if (! get_term_meta($result, $key)) {
                    // Save meta.
                    $resultMeta = add_term_meta($result['term_id'], $key, $value, true);
                    // To debugging purpose.
                    if (is_wp_error($resultMeta)) {
                        $this->logger->debug($resultMeta->get_error_message());
                    }
                }
            }
        } else {
    ...
    protected function fetch_remote_file($url, $post)
    {
        ...
        if (! empty($max_size) && $filesize > $max_size) {
            unlink($upload['file']);
            $message = sprintf(__('Remote file is too large, limit is %s', 'wordpress-importer'),
    ...
    protected function replace_attachment_urls_in_content()
    {
        ...
        foreach ($this->url_remap as $from_url => $to_url) {
            ...
            $query = $wpdb->prepare("UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s)", $from_url,
                $to_url);
            $wpdb->query($query);
    ...
    protected function prefill_existing_posts()
    {
        ...
        foreach ($posts as $item) {
            $unique                        = $this->makeUniqueGUID($item->guid);
            $this->exists['post'][$unique] = $item->ID;
        }
    }
    ...
    protected function post_exists($data)
    {
        ...
        $exists_key = $this->makeUniqueGUID($data['guid']);
