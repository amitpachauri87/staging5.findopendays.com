...
class PackageFormBuilder
{
    ...
    protected function getFieldsDefList()
    {
        ...
        return array_merge(
            ...
            include Plugin::getPluginDirPath('/inc/listingFormFields/calendarFields.php'),
            ...
            include Plugin::getPluginDirPath('/inc/listingFormFields/ticketFields.php')
        );
    }
    ...
    {
        $fieldFactory = new PluginFieldFactory();

        // Set the properly submit label.
        switch ($this->action) {
            case 'edit':
                $label = esc_html__('Submit Changes', 'qibla-events');
                break;
            default:
                $label = esc_html__('Create my Open Day', 'qibla-events');
                break;
        }
    ...
    public function setCurrentLang()
    {
        ...
        $this->formFactory->getForm()->addHidden(new Hidden(array(
            ...
            'attrs' => array(
                'value' => Ffw\isWpMlActive() && defined('ICL_LANGUAGE_CODE') ? ICL_LANGUAGE_CODE : '',
            ),
        )));
