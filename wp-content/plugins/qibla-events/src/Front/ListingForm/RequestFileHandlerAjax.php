...
use AppMapEvents\Debug\Exception;
...
class RequestFileHandlerAjax extends AjaxStoreMediaFile
{
    ...
    protected function getFields()
    {
        ...
        // @codingStandardsIgnoreStart
        $package = Fw\getPostByName(Fw\filterInput($_POST, PackageFormBuilder::FORM_PACKAGE_KEY), 'event_package');
                    ...
                    try {
                                ...
                                if (! $this->isSubTask('sync_media_ids')) {
                                    ...
                                    if ($response) {
                                        $response = new ResponseAjax(
                                            201,
                                            esc_html__('Files stored successfully.', 'qibla-events')
                                        );
                                    }
                                }
                                break;
                        ...
                        if ((($response instanceof Response && $response->isValidStatus()) ||
                            ...
                            if ($this->isSubTask('sync_media_ids')) {
                                $response = new ResponseAjax(
                                    200,
                                    esc_html_x('Files Sync success.', 'listing-submit', 'qibla-events')
                                );
                            }
                        }
                    } catch (\Exception $e) {
                        ...
                        $response = new ResponseAjax(
                            418,
                            esc_html__('Some file return an error during upload. Aborting', 'qibla-events')
                        );
                    }//end try
                endforeach;
            endif;
