...
namespace AppMapEvents\Scripts;
...
class LocalizeScripts extends ScriptAbstract
{
    ...
    public static function lazyLocalize($path, $action)
    {
        add_action($action, function () use ($path) {
            $instance = new LocalizeScripts(include \AppMapEvents\Plugin::getPluginDirPath($path));
    ...
    public function insertLocalizedScripts()
    {
        ...
        foreach ($list as $item) :
            ...
            if ($print) {
                 ...
                 */
                $item = apply_filters('appmap_ev_fw_insert_localized_script_item', $item);
