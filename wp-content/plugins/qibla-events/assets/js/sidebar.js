...
;(
    function (window, _, $) {
        ...
        var viewPortWidth  = Math.round(window.innerWidth),
            ...
            if (sidebarWrap) {
                var sidebarMarginTop = parseInt(articleHeader.offsetHeight / 2 - sidebarWrap.offsetHeight / 2);
            }
        /**
         * Render
         *
            ...
            if (1024 < viewPortWidth) {
                setTimeout(function () {
                    var main        = document.getElementById('dlmain'),
                        ...
                        title       = document.querySelector('.dlarticle__title'),
                        sidebar     = document.getElementById('dlsidebar-listings');
                    ...
                    // Set Position.
                    if (!isInViewport(title) && parseInt(sidebarRect.top) <= 0) {
                        sidebarWrap.classList.add('fixed');
                    }
                    if (parseInt(sidebarRect.top) > 0 &&
                        parseInt(sidebarRect.top) <= parseInt(title.getBoundingClientRect().top)
                    ) {
                        sidebarWrap.classList.remove('fixed');
                    }
                    ...
                    if (parseInt(main.getBoundingClientRect().bottom) <= parseInt(sidebarRect.bottom)) {
                        ...
                        sidebarWrap.style.marginTop = parseInt(sidebarParentRect.height - sidebarRect.height - 120) + 'px';
                    }
                    if (sidebarWrap.getBoundingClientRect().y > 120) {
                        sidebarWrap.classList.remove('stop');
                        sidebarWrap.style.marginTop = '';
                    }
        ...
        window.addEventListener('load', function () {
            ...
            if (sidebarWrap) {
                ...
                if (1024 < viewPortWidth && sidebarWrap.classList.contains('sidebar-sticky')) {
                    sidebarWrap.style.width = width + 'px';
                    sidebar.style.marginTop = parseInt(articleHeader.offsetHeight / 3) + 'px';
                    if (! widget) {
