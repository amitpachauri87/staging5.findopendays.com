<?php
namespace Qibla\TemplateEngine;

/**
 * Template Engine
 *
 * @since      1.0.0
 * @package    Qibla\Scripts
...
class Engine
{
    ...
    public function render()
    {
         ...
         * @param string    $name The name of the current template instance.
         */
        $data = apply_filters('qibla_template_engine_data', $this->data, $this->name);
         ...
         * @param \stdClass $data The template data.
         */
        $data = apply_filters("qibla_template_engine_data_{$this->name}", $data);
        ...
        // Retrieve the theme file path child and parent theme.
        $viewPath = get_theme_file_path($this->templatePath);

        /**
         * Filter View File Path
         *
         * @since 1.0.0
         *
         * @param string $viewPath The path of the current view to include.
         * @param string $viewName The name of the current view to include.
         */
        $viewPath = apply_filters("qibla_template_engine_view_path", $viewPath, $this->name);
         ...
         * @param string $viewPath The path of the current view to include.
         */
        $viewPath = apply_filters("qibla_template_engine_view_{$this->name}_path", $viewPath);

        // Empty string or bool depend by the conditional above.
        if (! file_exists($viewPath)) {
            throw new \Exception(sprintf(
                'Qibla Template Engine: No way to locate the template %s.',
                $viewPath
            ));
        }
