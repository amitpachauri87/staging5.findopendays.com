...
<div class="cart_totals <?php echo WC()->customer->has_calculated_shipping() ? ' calculated_shipping' : '' ?>">
    ...
    <div <?php F\scopeClass('cart-totals') ?>>
        <h2  <?php F\scopeClass('cart-totals', 'title') ?>>
            <?php esc_html_e('Cart Totals', 'qibla'); ?>
        </h2>
        <table cellspacing="0" class="shop_table shop_table_responsive">

            <tr class="cart-subtotal">
                <th>
                    <?php esc_html_e('Subtotal', 'qibla'); ?>
                </th>
                <td data-title="<?php esc_attr_e('Subtotal', 'qibla'); ?>">
            ...
            <?php elseif (WC()->cart->needs_shipping() && 'yes' === get_option('woocommerce_enable_shipping_calc')) : ?>
                ...
                <tr class="shipping">
                    <th>
                        <?php esc_html_e('Shipping', 'qibla'); ?>
                    </th>
                    <td data-title="<?php esc_attr_e('Shipping', 'qibla'); ?>">
            ...
            <?php if (wc_tax_enabled() && 'excl' === WC()->cart->tax_display_cart) :
                ...
                $estimated_text = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()
                    ? sprintf(' <small>(' . __('estimated for %s', 'qibla') . ')</small>',
                    ...
                    <?php foreach (WC()->cart->get_tax_totals() as $code => $tax) : ?>
                        <tr class="tax-rate tax-rate-<?php echo sanitize_title($code); ?>">
                            ...
                            <td data-title="<?php echo esc_attr($tax->label); ?>">
                                <?php echo F\ksesPost($tax->formatted_amount); ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
            ...
            <tr class="order-total">
                ...
                <td data-title="<?php esc_attr_e('Total', 'qibla'); ?>">
