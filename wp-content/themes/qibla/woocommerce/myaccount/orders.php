...
<?php if ($has_orders) : ?>
                    ...
                    <td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr($column_id); ?>"
                        ...
                        <?php elseif ('order-number' === $column_id) : ?>
                            <a href="<?php echo esc_url($order->get_view_order_url()); ?>">
                                <?php echo _x('#', 'hash before order number', 'qibla') . $order->get_order_number(); ?>
                            </a>
                        ...
                        elseif ('order-total' === $column_id) :
                            echo sprintf(
                                _n('%s for %s item', '%s for %s items', $item_count, 'qibla'),
                                $order->get_formatted_order_total(),
                                $item_count
                            );
                        elseif ('order-actions' === $column_id) :
                            // Actions for the current order.
                            Wf\orderActionsTmpl(wc_get_account_orders_actions($order), $order);
                        endif;
                        ?>
                    </td>
                <?php endforeach; ?>
    ...
    <?php if (1 < $customer_orders->max_num_pages) : ?>
        <div class="dladjacent-orders">
            <?php if (1 !== $current_page) : ?>
                <div class="dladjacent-orders__prev">
                    <a class="dladjacent-orders__link"
                       href="<?php echo esc_url(wc_get_endpoint_url('orders', $current_page - 1)); ?>">
                        <?php _e('Previous Orders', 'qibla'); ?>
                    </a>
                </div>
            <?php endif; ?>

            <?php if (intval($customer_orders->max_num_pages) !== $current_page) : ?>
                <div class="dladjacent-orders__next">
                    <a class="dladjacent-orders__link"
                       href="<?php echo esc_url(wc_get_endpoint_url('orders', $current_page + 1)); ?>">
                        <?php _e('Next Orders', 'qibla'); ?>
                    </a>
                </div>
            <?php endif; ?>
        </div>
...
<?php else : ?>
    <div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
        <a class="woocommerce-Button dlbtn dlbtn--small"
           href="<?php echo esc_url(
               apply_filters('woocommerce_return_to_shop_redirect', wc_get_page_permalink('shop'))
           ); ?>">
            <?php esc_html_e('Go Shop', 'qibla') ?>
        </a>
        <?php esc_html_e('No order has been made yet.', 'qibla'); ?>
    </div>
<?php endif; ?>
